...
class AMPCLearner(object):
    ...
    def model_rollout_for_update(self, start_obses, ite, mb_ref_index, mb_batch_obs):  # start_obses.shape = (256, 41)
        ...
        print(f'In ampc lines 88 the shape of the input mb_batch_obs of function model_rollout_for_update is {mb_batch_obs.shape}\n ')
    ...
    def compute_gradient(self, samples, rb, iteration):
        ...
        print(f'In ampc lines 189 the shape of mb_obs is {mb_obs.shape}')
        print(f'In ampc lines 191 the type of mb_obs is {type(mb_obs)}')
        mb_batch_obs = self.tf.constant(self.batch_data_lstm['batch_obs'])  # mb_batch_obs.shape = (256, 29, 41)
        print(f'In ampc lines 190 the shape of mb_batch_obs is {mb_batch_obs.shape}')
        print(f'In ampc lines 191 the type of mb_batch_obs is {type(mb_batch_obs)}')
        # print(f'In ampc lines 191 the first element of mb_batch_obs is {mb_batch_obs[0]}')
        # print(f'In ampc lines 192 the type of mb_batch_obs is {type(mb_batch_obs)}')
